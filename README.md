...
| 已安装的正式版本 | 静默检查并在有新版本时后台下载 | 读取安装包内当前版本说明 | 直接读取更新清单；有新版时立即开始差分下载并在按钮背景显示进度 |
| 静态网页预览 | 不支持 | 入口隐藏 | 入口隐藏 |

后台下载过程中，“检查更新”按钮会用背景填充和百分比同步展示进度；下载完成后按钮变为“快速重启更新 Vx.x.x”。点击后旧窗口立即收起，更新在后台静默覆盖安装并自动打开新版。如果暂不点击，更新会在应用正常退出时安装，下次启动直接进入新版。安装版直接读取 `latest.yml`，避免额外依赖 GitHub Release API 导致检查与下载状态脱节。
