...
idle
  → checking
  → available
  → downloading（后台进行，按钮隐藏）
  → downloaded（显示更新按钮，可点击安装）
  → installing（隐藏旧窗口、静默安装、重新启动）
```

检查或下载失败时保持入口隐藏，不打断日历；下一次启动会重新检查。安装失败时恢复已下载状态，允许再次点击安装。开发模式不自动下载，界面不提供手动检查入口；诊断脚本仍可调用保留的检查接口。
...
界面使用 `stateRevision`（状态序号）处理启动快照、安装请求与事件之间的乱序。请求开始时记住序号，收到有效事件或开始新操作时递增；请求返回时若序号不同，就不能再修改界面。例如安装失败后的恢复事件先到、安装请求的旧返回后到，旧返回不能再次锁住按钮。
