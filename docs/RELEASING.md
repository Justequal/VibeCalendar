...
用户明确要求“更新版本”时，默认授权完成本地修改、验证、版本号与更新日志维护、推送远程分支与对应版本标签，并检查 GitHub Release 和安装包发布结果。不能只停留在本地构建，也无需重复询问是否推送或发布。
...
- [ ] NSIS 使用机器级安装记录，更新前后安装目录保持不变
- [ ] 应用底部显示正确版本号
- [ ] 点击版本号能看到当前安装版本对应的维护记录，而不是远端其他版本的说明
- [ ] 在旧的已安装正式版本中，启动后能后台检查和下载新版本，完成前按钮隐藏，完成后可点击更新
