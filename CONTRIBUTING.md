...
修改模块边界时同步更新[学习路径](docs/learning/README.md)与架构说明。公开函数或复杂分支应说明输入、输出、副作用、异常和设计理由，提供具体例子；避免逐行翻译语法。新增技术方法先判断是否解决生产问题，否则放入独立实验并标明用途和限制。

实验须离线、确定性、可重复，包含行为断言，不访问用户数据；`npm run verify`会检查实验入口。修改界面连接后另运行 `npm run test:ui`。保持安装包范围与现有功能，教学重构本身不触发发布。
